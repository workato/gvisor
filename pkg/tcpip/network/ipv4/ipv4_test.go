...
func buildTestRoute(t *testing.T, packetCollectorErrors []*tcpip.Error, mtu uint32) testRoute {
...
func TestFragmentation(t *testing.T) {
	...
	for _, ft := range fragTests {
		...
		L:
			...
			if got, want := len(results), int(r.Stats().IP.PacketsSent.Value()); got != want {
...
func TestFragmentationErrors(t *testing.T) {
	...
	for _, ft := range fragTests {
		t.Run(ft.description, func(t *testing.T) {
			...
			r := buildTestRoute(t, ft.packetCollectorErrors, ft.mtu)
			...
			if got, want := r.linkEP.Drain(), int(r.Stats().IP.PacketsSent.Value())+1; err != nil && got != want {
