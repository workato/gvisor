...
// Funciton sendICMPError sends an ICMP error report back to the remote device
// that sent the problematic packet. It will incorporate as much of that packet
// as possible as well as any error metadata as is available.
// SendICMPError can only be called from within code that is knowledgeable about
// IPv6 ICMP but there is no reason to not export it should the need arise for
// it to be called outside this package. For Protocol agnostic code, call
// protocol.ReturnError.
