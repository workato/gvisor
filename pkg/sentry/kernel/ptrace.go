...
func (t *Task) Ptrace(req int64, pid ThreadID, addr, data usermem.Addr) error {
	...
	case linux.PTRACE_PEEKTEXT, linux.PTRACE_PEEKDATA:
		...
		if _, err := word.CopyIn(t.AsCopyContext(usermem.IOOpts{IgnorePermissions: true}), addr); err != nil {
			return err
		}
	...
	case linux.PTRACE_POKETEXT, linux.PTRACE_POKEDATA:
		...
		_, err := word.CopyOut(t.AsCopyContext(usermem.IOOpts{IgnorePermissions: true}), addr)
		return err
