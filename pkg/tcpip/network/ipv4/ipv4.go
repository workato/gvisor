...
func (e *endpoint) GetAssignedEndpoint(localAddr tcpip.Address, spoofingOrPromiscuous bool, tempPEB stack.PrimaryEndpointBehavior) stack.AddressEndpoint {
	e.mu.Lock()
	defer e.mu.Unlock()

	if r := e.mu.ep.GetAssignedEndpoint(localAddr, spoofingOrPromiscuous, tempPEB); r != nil {
		return r
	}
	...
	for _, r := range eps {
		...
		subnet := addr.Subnet()
		if subnet.IsBroadcast(localAddr) && r.IsAssigned(spoofingOrPromiscuous) && r.IncRef() {
			return r
		}
	}
