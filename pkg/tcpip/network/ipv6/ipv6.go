...
func (e *endpoint) HandlePacket(r *stack.Route, pkt *stack.PacketBuffer) {
	...
	for firstHeader := true; ; firstHeader = false {
		...
		case header.IPv6HopByHopOptionsExtHdr:
			...
			if !firstHeader {
				sendICMPError(r, header.ICMPv6ParamProblem, header.ICMPv6UnknownHeader, it.HeaderOffset()+it.ParseOffset(), pkt)
				return
			}
			...
			for {
				...
				case header.IPv6OptionUnknownActionDiscardSendICMP:
					sendICMPError(r, header.ICMPv6ParamProblem, header.ICMPv6UnknownOption, it.HeaderOffset()+it.ParseOffset(), pkt)
					return
		...
		case header.IPv6RoutingExtHdr:
			...
			if extHdr.SegmentsLeft() != 0 {
				sendICMPError(r, header.ICMPv6ParamProblem, header.ICMPv6ErroneousHeader, it.HeaderOffset()+it.ParseOffset(), pkt)
				return
			}
